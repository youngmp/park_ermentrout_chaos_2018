...
class Theta(object):
    ...
    def __init__(self,
        ...
        """
        heterogeneous_pop: use heterogeneous population (see ode/theta2_het_pop.ode)
    ...
    def run_full_sim(self):
        ...
        if self.N == 1:
            ...
            if self.heterogeneous_pop:
                fname = 'ode/theta1_het_pop.ode'
            else:
                fname = 'ode/theta1_het.ode'
                
            npa, vn = xpprun(fname,
                             xppname='xppaut',
                             inits={'x0':self.x[0,0],'y0':self.y[0,0],
            ...
            if self.heterogeneous_pop:
                fname = 'ode/theta'+str(n)+'_het_pop.ode'

            else:
                fname = 'ode/theta'+str(n)+'_het.ode'
        ...
        #num_opts = read_numerics('ode/theta2_het.ode')
    ...
    def plot(self,choice='full',nskip=1):
        ...
        if choice == 'full-vars':
        ...
        elif choice == 'phase-diffdiff-raw':
            ax = plt.subplot(111)
            ax.set_title("phase diff raw (numerics)")
            ...
            for i in range(self.N-1):
                ...
                diff2 = self.phasey[:,i+1]-self.phasey[:,0]

                diffdiff1 = diff1-diff3
                diffdiff2 = diff2-diff3

                diffdiff1 = np.mod(diffdiff1+self.perx/2.,self.perx)-self.perx/2.
                diffdiff2 = np.mod(diffdiff2+self.pery/2.,self.pery)-self.pery/2.

                #diff1 = np.mod(+self.perx/2.,self.perx)-self.perx/2.
                #diff2 = np.mod(+self.pery/2.,self.pery)-self.pery/2.
                #print i,str(i+2),(.5*i+.5)/self.N
                #ax.scatter(self.t[::nskip],diff1[::nskip],color='blue',edgecolor='none',label='x'+str(i+2)+'-x1',alpha=(.5*(i+1)+.5)/self.N)
                #ax.scatter(self.t[::nskip],diff2[::nskip],color='green',edgecolor='none',label='y'+str(i+2)+'-y1',alpha=(.5*(i+1)+.5)/self.N)
                #ax.scatter(self.t[::nskip],diff1[::nskip],color=blues[i],edgecolor='none',label='x'+str(i+2)+'-x1')

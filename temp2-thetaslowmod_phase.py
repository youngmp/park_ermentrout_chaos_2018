...
class Phase(object):
    ...
    def plot(self,choice='h_inhom',brute=False,nskip=1):
        ...
        if choice == 'h_inhom':
            ...
            datdom = np.linspace(0,self.ta[-1]/2.,5)
            ...
            for i in range(len(datdom)):
                ...
                dom,tot = self.generate_h(datx[i],daty[i],choice='xx',return_domain=True,brute=brute)
                ax1.plot(dom,tot,color=c,picker=5)
                #ax1.set_xlim(dom[0],dom[-1])


                dom,tot = self.generate_h(datx[i],daty[i],choice='yy',return_domain=True,brute=brute)
                ax2.plot(dom,tot,color=c,picker=5)
                #ax1.set_xlim(dom[0],dom[-1])
            ...
            for i in range(self.N-1):
                ...
                ax.scatter(self.t[::nskip],diff1,color='blue', edgecolor='none',label='thx'+str(i+2)+'-thx1')
                ax.scatter(self.t[::nskip],diff2,color='green',edgecolor='none',label='thy'+str(i+2)+'-thy1')
